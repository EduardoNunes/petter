...
const MessageToast: React.FC<MessageToastProps> = ({ textError, setToast }) => {
  ...
  const [bgColor, setBgColor] = useState("amareloPadrao");
  const sucesso = textError.includes("sucesso");

  useEffect(() => {
    if (sucesso === true) {
      setBgColor("verdeForte");
    }
  }, []);
  ...
  return (
    <div className="absolute top-[50%] translate-x-[-50%] left-[50%] translate-y-[-50%] z-10">
      <div
        className={`flex items-center text-center h-auto w-auto px-4 py-4 rounded-lg z-1 bg-${bgColor}`}
      >
